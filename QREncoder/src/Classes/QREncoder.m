...
////////////////////////////////////////////////////////////////////////////////////////////////////
+ (UIImage *)imageForMatrix:(QRMatrix *)matrix withIconImage:(UIImage *)iconImage size:(int)size
{
    UIImage *tempImage = [QREncoder imageForMatrix:matrix];
    UIGraphicsBeginImageContext(tempImage.size);
    CGRect firstRect = CGRectMake(0, 0, tempImage.size.width, tempImage.size.height);
    CGRect secondRect = CGRectMake(tempImage.size.width / 2 - size / 2, tempImage.size.height / 2 - size / 2, size, size);
    [tempImage drawInRect:firstRect];
    [iconImage drawInRect:secondRect];
...
////////////////////////////////////////////////////////////////////////////////////////////////////
+ (UIImage *)imageForMatrix:(QRMatrix *)matrix withIconImage:(UIImage *)iconImage
{
    return [QREncoder imageForMatrix:matrix withIconImage:iconImage size:20];
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////////
+ (UIImage *)encode:(NSString *)str withImage:(UIImage *)image;
{
    return [QREncoder encode:str mainSize:4 withImage:image forSize:20 correctionLevel:QRCorrectionLevelHigh];
}

///////////////////////////////////////////////////////////////////////////////////////////////////
+ (UIImage *)encode:(NSString *)str mainSize:(int)mainSize withImage:(UIImage *)icon forSize:(int)iconSize correctionLevel:(QRCorrectionLevel)level;
{
    QREncoder *encoder[8];
    for (int i = 0; i < 8; i ++)
    {
        encoder[i] = [[QREncoder alloc] initWithStr:str size:mainSize correctionLevel:level pattern:i];
        [encoder[i] encode];
    }
    
    int minLostPoint = LONG_MAX;
    QREncoder *tempEncoder = nil;
    for (int i = 0; i < 8; i ++)
    {
        if (nil != encoder[i]->_matrix)
        {
            int lostPoint = [encoder[i] lostPoint];
            if (lostPoint < minLostPoint)
            {
                minLostPoint = lostPoint;
                tempEncoder = encoder[i];
            }
        }
    }
    ...
    if (nil != tempEncoder)
    {
        image = [QREncoder imageForMatrix:tempEncoder->_matrix withIconImage:icon size:iconSize];
    }
    else
    {
        image = nil;
  ...
  for(int r = -1; r <= 7; r++) {
    ...
    for(int c = -1; c <= 7; c++) {
      ...
      BOOL bit = (r >= 0 && r <= 6 && (c == 0 || c == 6)) || 
        (c >= 0 && c <= 6 && (r == 0 || r == 6)) ||
        (r >= 2 && r <= 4 && c >= 2 && c <= 4);
        NSLog(@"row \t column \t r \t c \t bit \n %d \t %d \t %d \t %d \t %d \t", row, col, r, c, bit);
