...
- (void)updateIndicators {
	...
	BOOL isRadialProgress = [indicator isKindOfClass:[MDRadialProgressView class]];
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		[indicator removeFromSuperview];
		self.indicator = nil;
	} else if (mode == MBProgressHUDRadialProgress)
    {
        if (!isRadialProgress)
        {
            [indicator removeFromSuperview];
            self.indicator = MYDEMOLIST_AUTORELEASE([[MDRadialProgressView alloc] init]);
...
- (void)internalInit
{
    ...
    self.incompletedColor = [UIColor redColor];
    self.sliceDividerColor = [UIColor whiteColor];
    self.thickness = 10;
    self.progress = 0.0f;
    self.progressTotal = 10.0f;
